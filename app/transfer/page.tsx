...
export default function TransferPage() {
  ...
  async function verifyRecipient() {
    ...
    try {
      ...
      if (verifiedRecipient?.isFrozen) {
        setError("This recipient account is unavailable.");
        return;
      }
  ...
  return <main className="min-h-screen p-4 sm:p-6"><div className="mx-auto max-w-3xl"><Link href="/" className="inline-flex items-center gap-2"><ArrowLeft size={16} /> Back</Link><div className="mt-6 rounded-2xl border p-6"><div className="flex items-center justify-between gap-4"><div><h1 className="text-2xl font-semibold">Transfer Money</h1><p className="mt-1 text-sm opacity-70">Send money securely from your Crestline Capital account.</p></div><ShieldCheck /></div>
    ...
    <form onSubmit={review} className="mt-6 space-y-5"><div className="rounded-xl border p-4"><div className="text-sm opacity-60">Available balance</div><div className="mt-1 text-xl font-semibold">{user?.balance ?? 0} {user?.currency ?? "USD"}</div></div>
      ...
      <div className="flex justify-end"><button type="submit" disabled={submitting} className="inline-flex items-center gap-2 rounded-xl border px-5 py-3 font-medium">Review Transfer <ArrowRight size={16}/></button></div>
    </form>
    {step === "review" && <div className="fixed inset-0 z-50 grid place-items-center bg-black/50 p-4"><div className="w-full max-w-lg rounded-2xl bg-background p-6"><h2 className="text-xl font-semibold">Review Transfer</h2><div className="mt-5 space-y-3 text-sm"><div className="flex justify-between"><span>From</span><span>{mask(user?.accountNumber ?? "")}</span></div><div className="flex justify-between"><span>To</span><span>{recipient?.name} · {mask(recipient?.accountNumber ?? "")}</span></div><div className="flex justify-between"><span>Amount</span><span>{numericAmount.toFixed(2)} {user?.currency ?? "USD"}</span></div><div className="flex justify-between"><span>Fee</span><span>{fee.toFixed(2)}</span></div><div className="flex justify-between font-semibold"><span>Total</span><span>{total.toFixed(2)}</span></div></div><div className="mt-6 flex justify-end gap-3"><button type="button" onClick={() => setStep("details")} className="rounded-xl border px-4 py-2">Edit</button><button type="button" onClick={submit} disabled={submitting} className="rounded-xl border px-4 py-2">{submitting ? "Sending…" : "Confirm Transfer"}</button></div></div></div>}
  </div></div></main>;
}
