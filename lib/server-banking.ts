...
const TransactionSchema = new Schema({
  ...
  type: { type: String, enum: ['internal', 'external', 'manual'], required: true },
...
const TransferRequestSchema = new Schema({
  userId: { type: Schema.Types.ObjectId, ref: 'User', required: true }, key: { type: String, required: true }, statusCode: { type: Number, required: true }, response: { type: Schema.Types.Mixed, required: true }, createdAt: { type: Date, default: Date.now, expires: 60 * 60 * 24 },
});
...
export function publicUser(user: UserDoc) {
  return { id: user._id.toString(), name: user.name, email: user.email, accountNumber: user.accountNumber, balance: Number(user.balance?.toString() || 0), isAdmin: Boolean(user.isAdmin), isFrozen: Boolean(user.isFrozen), status: user.status || (user.isFrozen ? 'suspended' : 'active'), accountState: user.accountState || 'active', kycStatus: user.kycStatus || 'pending', emailVerified: Boolean(user.emailVerified), twoFactorEnabled: Boolean(user.twoFactorEnabled), forcePinCheck: Boolean(user.forcePinCheck), restrictedMessages: user.restrictedMessages || {}, lastLoginAt: user.lastLoginAt, createdAt: user.createdAt };
}
...
export async function authenticatedUser(request: Request): Promise<UserDoc> {
  const header = request.headers.get('authorization') || ''; const token = header.startsWith('Bearer ') ? header.slice(7) : ''; const secret = process.env.JWT_SECRET;
  if (!token || !secret) throw new Error('UNAUTHORIZED'); const payload = jwt.verify(token, secret) as { userId?: string }; if (!payload.userId) throw new Error('UNAUTHORIZED');
  const user = await User.findById(payload.userId) as UserDoc | null; if (!user) throw new Error('UNAUTHORIZED'); return user;
}
