...
export default defineSchema({
  users: defineTable({
    authUserId: v.string(), legacyId: v.optional(v.string()), email: v.string(), name: v.string(), accountNumber: v.string(),
    role: v.union(v.literal("USER"), v.literal("ADMIN")), isFrozen: v.boolean(), pinHash: v.optional(v.string()), createdAt: v.number(),
  }).index("by_authUserId", ["authUserId"]).index("by_email", ["email"]).index("by_accountNumber", ["accountNumber"]).index("by_legacyId", ["legacyId"]),

  accounts: defineTable({ userId: v.id("users"), type: v.string(), currency: v.string(), availableBalance: money, createdAt: v.number() }).index("by_user", ["userId"]),
  ...
  }).index("by_reference", ["reference"]).index("by_idempotency", ["senderAccountId", "idempotencyKey"]).index("by_sender", ["senderAccountId"]).index("by_receiver", ["receiverAccountId"]).index("by_legacyId", ["legacyId"]),

  walletLedger: defineTable({ accountId: v.id("accounts"), userId: v.id("users"), transactionId: v.id("transactions"), amount: money, currency: v.string(), direction: v.union(v.literal("DEBIT"), v.literal("CREDIT")), type: v.string(), status: v.string(), reference: v.string(), eventId: v.optional(v.string()), createdAt: v.number() }).index("by_transaction", ["transactionId"]).index("by_account", ["accountId"]).index("by_user", ["userId"]).index("by_eventId", ["eventId"]),
  ...
  webhookEvents: defineTable({ eventId: v.string(), eventType: v.string(), status: v.string(), reference: v.optional(v.string()), createdAt: v.number(), processedAt: v.optional(v.number()) }).index("by_eventId", ["eventId"]),
});
